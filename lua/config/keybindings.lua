...
function M.setup()
    ...
    map("i", "<F9>", '<cmd>lua require("dap").toggle_breakpoint()<CR>', opt)
    ...
    -- `dap.close()` only drops the client connection and leaves the debuggee
    -- running; `terminate()` is what actually ends the session.
    map("n", "<leader>ds", '<cmd>lua require("dap").terminate()<CR>', opt)
    ...
    -- Overseer (build / test tasks). `<leader>o` was unused.
    map("n", "<leader>ob", "<cmd>OverseerRun<CR>", opt)
    map("n", "<leader>oo", "<cmd>OverseerToggle<CR>", opt)
    map("n", "<leader>oq", "<cmd>OverseerQuickAction<CR>", opt)
    map("n", "<leader>oc", "<cmd>OverseerRunCmd<CR>", opt)
